...
std::vector<blink::type> blink::pdb_reader::types()
{
	...
	const auto read_num = [&stream]() -> unsigned int
	{
		const auto value = stream.read<uint16_t>();
	...
	const auto encode_num = [](size_t num) -> std::string
	{
		if (num == 0)
		{
			return "A@";
	...
	const auto decode_num = [](std::string &data) -> size_t
	{
		size_t result = 0;
	...
	for (auto current_index = header.min_index; current_index < header.max_index; current_index++)
	{
		...
		switch (tag)
		{
			...
			case 0x1503: // LF_ARRAY
			{
				...
				const auto actual_size = read_num();
...
std::vector<std::string> blink::pdb_reader::buildtools()
{
	// Read build info
